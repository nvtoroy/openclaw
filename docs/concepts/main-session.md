...
- By default the session resets daily at 04:00 local time (configurable, or
  ...
  notes, and the next session re-primes recent notes. Reset assigns a new live
  session id but keeps the previous SQLite transcript searchable under the
  same main-session key.
